...
const quizData = [
  { 
    q: "Quanti passaggi compongono l'handshake iniziale di TCP?", 
    a: ["2 passaggi", "3 passaggi", "4 passaggi"], 
    c: 1 
  },
  { 
    q: "Qual è la funzione principale del flag SYN?", 
    a: ["Terminare la connessione", "Sincronizzare i numeri di sequenza", "Confermare la ricezione dei dati"], 
    c: 1 
  },
  { 
    q: "Cosa succede se un segmento TCP non viene confermato (ACK) entro un certo tempo?", 
    a: ["Viene scartato per sempre", "Viene ritrasmesso dal mittente", "La connessione viene chiusa immediatamente"], 
    c: 1 
  },
  { 
    q: "Quale meccanismo TCP evita di inondare il ricevitore con troppi dati?", 
    a: ["Checksum", "Controllo di Flusso (Windowing)", "Handshake"], 
    c: 1 
  },
  { 
    q: "Qual è la sequenza corretta per la chiusura di una connessione TCP?", 
    a: ["FIN -> ACK -> FIN -> ACK", "SYN -> SYN-ACK -> ACK", "DATA -> ACK"], 
    c: 0 
  }
];

export default function TCPQuiz() {
      ...
      <AnimatePresence mode="wait">
        {state === 'start' && (
          ...
          >
            ...
            <h2 className="text-3xl font-black text-slate-900 mb-4">Verifica Competenze TCP</h2>
            <p className="text-slate-600 mb-8 leading-relaxed">
              Metti alla prova la tua conoscenza sull'handshake a 3 vie, l'affidabilità e il controllo di flusso di TCP.
            </p>
            <button 
              onClick={handleStart}
              className="px-10 py-4 bg-orange-600 text-white rounded-2xl font-bold hover:bg-orange-700 transition-all shadow-lg shadow-orange-200"
            >
              Inizia il Quiz
            </button>
          </motion.div>
                ...
                {quizData.map((_, i) => (
                  <div 
                    ...
                    className={cn(
                      ...
                      i < currentIndex ? "bg-orange-600" : i === currentIndex ? "bg-orange-200" : "bg-slate-100"
                    )} 
                  />
                ))}
              </div>
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      selectedOption === null 
                        ? "border-slate-100 hover:border-orange-500 hover:bg-orange-50/50" 
                        : isSelected
          ...
          >
            ...
            <p className="text-slate-600 mb-10">
              {score === quizData.length 
                ? "Eccellente! Conosci TCP come le tue tasche." 
                : score >= 3 
                  ? "Ben fatto! Hai compreso i meccanismi fondamentali di TCP." 
                  : "TCP è complesso. Ti consigliamo di studiare meglio l'handshake e il controllo di flusso."}
            </p>
